...
| 0008 | Transcrição da busca por voz por modelo Whisper local | reavalia o argumento "modelo local = gigabytes no instalador" da ADR-0006 |
...
└───┬──────────────┬──────────────┬───────┘
    ▼              ▼              ▼
    GitHub API   Banco local   utilityProcess de voz (Whisper local, ADR-0008)
```
...
* **A busca aceita ditado por voz (ADR-0008).** Um ícone de microfone no `<form className="busca">` — só visível com a busca por voz ativada nas configurações e o modelo baixado. O renderer captura o áudio (`getUserMedia`), reamostra para 16 kHz mono e manda por IPC (`voz:transcrever`); a transcrição roda num **`utilityProcess`** (`main/voz/`) com `@huggingface/transformers` + `onnxruntime-node` CPU sobre o `onnx-community/whisper-small` q8, **baixado sob demanda** para `userData/modelos` (nunca no instalador). O áudio **não sai da máquina** e não é gravado — a ADR-0006 não se aplica. O transcrito volta ao renderer, **preenche o campo e não dispara a busca**: é a fala do usuário, não conteúdo de documento (ADR-0005 não incide). Permissão de microfone: só áudio, só a janela da aplicação (`main/permissoes.ts`); no primeiro acionamento um modal do renderer pede a confirmação de uso (grava `voz.microfoneConsentido` via `voz:microfone`) antes de qualquer `getUserMedia`. O microfone de captura é escolhível nas configurações (`voz.microfone` = `deviceId`; ausente = padrão do sistema, e a captura recai nele se o escolhido sumir). Parâmetros e manifesto de integridade em `AncorAI/instrucoes/transcricao.md`, lidos em runtime.
...
Banco NoSQL orientado a documentos, no processo main, isolado em um único módulo (`banco/repositorio.ts`). Coleções: `documentos_acessados` (só o link de redirecionamento), `cache_fontes` (respostas de API com `ETag`), `conteudo_documentos` (texto extraído, ADR-0005), `acervo_documentos` (snapshot do inventário com autoria/data resolvidas) e `preferencias` (decisões lembradas, `boolean` ou `string`: consentimento de envio à LLM, `voz.ativa`, `voz.microfoneConsentido`, `voz.microfone` = `deviceId` escolhido para o ditado). O texto e os bytes originais nunca chegam ao renderer; os bytes não são guardados em lugar nenhum. Os campos de resumo já existem, reservados para a IA futura.
