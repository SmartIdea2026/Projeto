...
const principal = semComentarios('src/main/index.ts');
...
describe('fronteira entre renderer e main', () => {
  ...
  it('mantém o isolamento de contexto e o Node fora do renderer', () => {
    expect(principal).toMatch(/contextIsolation:\s*true/);
    expect(principal).toMatch(/nodeIntegration:\s*false/);
  });
  ...
  it('o canal de gravação de credencial responde com status, não com o segredo', () => {
    ...
    const inicio = principal.indexOf('CANAIS.credenciaisDefinir');
    expect(inicio).toBeGreaterThan(-1);
    const trecho = principal.slice(inicio);
