...
export function App() {
  ...
  const carregarRecentes = useCallback(
    async (filtrosAtuais: TipoFiltros, emSegundoPlano = false) => {
      ...
      try {
        const novo = await window.ancoria.recentes(filtrosAtuais);
  ...
  const executarBusca = useCallback(async (filtrosAtuais: TipoFiltros) => {
    setCarregando(true);
    try {
      setResultado(await window.ancoria.buscar(filtrosAtuais));
    } finally {
      setCarregando(false);
    }
  }, []);
  ...
  useEffect(() => {
    void (async () => {
      ...
      const guardado = await window.ancoria.recentesDoCache(FILTROS_PADRAO);
      if (guardado && guardado.documentos.length > 0) {
        setResultado(guardado);
        setCarregando(false);
      }

      const atual = await window.ancoria.status();
      setStatus(atual);
  ...
  async function abrir(documento: Documento) {
    await window.ancoria.abrirDocumento(documento);
  }
      ...
      <main className="conteudo">
        ...
        {!carregando && !temCredencial && (
          <div className="vazio">
            ...
            <p>
              O AncorIA precisa de uma credencial do GitHub ou de uma conexão com o
